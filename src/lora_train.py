...
def train(args, model, accelerator, metrics_dict, 
    ...
    for epoch in range(args.max_train_epochs):
        ...
        for batch in epoch_iterator:
            with accelerator.accumulate(model):
                ...
                if args.wandb:
                    wandb.log(
                        {"train/loss": loss.item(), "train/epoch": epoch},
                        step=global_steps,
                    )
        ...
        model.eval()
        with torch.no_grad():
            val_loss, val_metric_dict = eval_loop(
                args, model, metrics_dict, val_loader, loss_fn, device
            )
            ...
            if args.wandb:
                ...
                wandb.log(val_log)
            print(
                f"EPOCH {epoch} VAL loss: {val_loss:.4f} {args.monitor}: {val_metric_score:.4f}"
            )
        ...
        if args.monitor == "loss":
            if val_loss < best_val_loss:
                ...
                torch.save(unwrap_model.state_dict(), path)
            ...
            if (
                len(val_loss_list) - val_loss_list.index(min(val_loss_list))
                > args.patience
            ):
            ...
            if val_metric_score > best_val_metric_score:
                ...
                print(
                    f">>> BEST at epcoh {epoch}, loss: {val_loss:.4f} {args.monitor}: {best_val_metric_score:.4f}"
                )
                for metric_name, metric_score in val_metric_dict.items():
                    print(f">>> {metric_name}: {metric_score:.4f}")
                print(f">>> Save model to {path}")
            ...
            if (
                len(val_metric_list) - val_metric_list.index(max(val_metric_list))
                > args.patience
            ):
                print(f">>> Early stopping at epoch {epoch}")
                break
    ...
    model.to(device)
    with torch.no_grad():
        test_loss, test_metric_dict = eval_loop(
            args, model, metrics_dict, test_loader, loss_fn, device
        )
        ...
        if args.wandb:
            ...
            wandb.log(test_log)
        print(
            f"EPOCH {epoch} TEST loss: {test_loss:.4f} {args.monitor}: {test_metric_score:.4f}"
        )
        for metric_name, metric_score in test_metric_dict.items():
            print(f">>> {metric_name}: {metric_score:.4f}")


def eval_loop(args, model, metrics_dict, dataloader, loss_fn, device=None):
    total_loss = 0
    ...
    for batch in epoch_iterator:
        ...
        for metric_name, metric in metrics_dict.items():
            if args.problem_type == "regression" and args.num_labels == 1:
                # ! logits [batch_size, 1], label [batch_size]
                loss = loss_fn(logits.squeeze(-1), label)
                metric(logits.squeeze(-1), label)
            elif args.problem_type == "multi_label_classification":
                loss = loss_fn(logits, label.float())
                metric(logits, label)
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="embedding hidden size of the model",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pooling_method", type=str, default="attention1d", help="pooling method"
    )
    parser.add_argument(
        "--pooling_dropout", type=float, default=0.25, help="pooling dropout"
    )

    # dataset
    parser.add_argument("--dataset", type=str, default=None, help="dataset name")
    parser.add_argument(
        "--dataset_config", type=str, default=None, help="config of dataset"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10000,
        help="max number of token per batch",
    )
    parser.add_argument(
        "--max_train_epochs", type=int, default=20, help="training epochs"
    )
    parser.add_argument(
        "--max_seq_len", type=int, default=None, help="max sequence length"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--structure_seqs", type=str, default=None, help="structure token"
    )
    parser.add_argument(
        "--loss_fn",
        type=str,
    ...
    # save model
    parser.add_argument(
        "--use_lora", action="store_true", help="use lora model"
    )  
    ...
    parser.add_argument(
        "--ckpt_root", default="ckpt", help="root directory to save trained models"
    )
    parser.add_argument(
        "--ckpt_dir", default=None, help="directory to save trained models"
    )
    # wandb log
            ...
            for m in args.metrics:
                if m == "accuracy":
                    if args.num_labels == 2:
                        ...
                        metrics_dict[m] = Accuracy(
                            task="multiclass", num_classes=args.num_labels
                        )
                elif m == "recall":
                    if args.num_labels == 2:
                    ...
                    if args.num_labels == 2:
                        ...
                        metrics_dict[m] = MatthewsCorrCoef(
                            task="multiclass", num_classes=args.num_labels
                        )
                elif m == "auc":
                    if args.num_labels == 2:
    ...
    if "esm" in args.plm_model:
        ...
        args.hidden_size = model.plm_model.config.hidden_size
    ...
    if args.train_file == None:
        train_dataset, train_token_num = process_dataset_from_list(
            load_dataset(args.dataset)["train"]
        )
        val_dataset, val_token_num = process_dataset_from_list(
            load_dataset(args.dataset)["validation"]
        )
        test_dataset, test_token_num = process_dataset_from_list(
            load_dataset(args.dataset)["test"]
        )

    if dataset_config["normalize"] == "min_max":
        train_dataset, val_dataset, test_dataset = min_max_normalize_dataset(
    ...
    def collate_fn(examples):
        ...
        for e in examples:
            ...
            # if "esm3_structure_seq" in args.structure_seqs:
            #     esm3_structure_seq = (
            #         [VQVAE_SPECIAL_TOKENS["BOS"]]
            #         + e["esm3_structure_seq"]
            #         + [VQVAE_SPECIAL_TOKENS["EOS"]]
            #     )
        ...
        if "ankh" in args.plm_model:
            ...
            if "esm1b" in args.plm_model:
                ...
                aa_inputs = tokenizer(
                    aa_seqs, return_tensors="pt", padding=True, truncation=True
                )
                
            if "foldseek_seq" in args.structure_seqs:
                foldseek_input_ids = tokenizer(
                    foldseek_seqs, return_tensors="pt", padding=True, truncation=True
                )["input_ids"]
            if "ss8_seq" in args.structure_seqs:
                ss8_input_ids = tokenizer(
                    ss8_seqs, return_tensors="pt", padding=True, truncation=True
                )["input_ids"]
    ...
    if args.problem_type == "single_label_classification":
        if args.loss_fn == "cross_entropy":
            ...
            alpha = [
                len(train_labels) / train_labels.count(i)
                for i in range(args.num_labels)
            ]
            print(">>> alpha: ", alpha)
            loss_fn = MultiClassFocalLossWithAlpha(
                num_classes=args.num_labels, alpha=alpha, device=device
            )
